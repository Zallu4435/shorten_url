...
export function CreateTunnelModal({ onClose, onTokenRevealed }: CreateTunnelModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setFormError(err instanceof Error ? err.message : "An unexpected error occurred during initialization.");
        }
    };
    ...
    const pythonCmd = `curl -sSL ${typeof window !== 'undefined' ? window.location.origin : ''}/agent/agent.py -o agent.py && python3 agent.py --alias ${alias} --token ${payload?.rawToken}`;
    const nodeCmd = `curl -sSL ${typeof window !== 'undefined' ? window.location.origin : ''}/agent/agent.js -o agent.js && node agent.js --alias ${alias} --token ${payload?.rawToken}`;
                                    ...
                                    <div className="space-y-5">
                                        <div className="space-y-2.5">
                                            ...
                                            <div className="relative group">
                                                ...
                                                <Input
                                                    className="h-14 pl-[144px] rounded-2xl border-border bg-muted/20 focus:bg-background focus:ring-primary/10 transition-all font-bold text-base tracking-tight text-foreground"
                            ...
                            {step === "token" && payload && (
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex items-center justify-between ml-1">
                                            ...
                                            <div className="flex bg-muted/40 p-1 rounded-xl border border-border">
                                                <button
                                                    ...
                                                    className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${platform === "python" ? "bg-background text-primary shadow-sm" : "text-muted-foreground/60 hover:text-muted-foreground"}`}
                                                >
                                                    Python
                                                </button>
                                                <button
                                                    onClick={() => setPlatform("node")}
                                                    className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${platform === "node" ? "bg-background text-primary shadow-sm" : "text-muted-foreground/60 hover:text-muted-foreground"}`}
                                                >
                                                    Node.js
                                                </button>
                                            </div>
                                        ...
                                        <p className="text-[10px] font-bold text-muted-foreground/40 px-1">
                                            This one-liner will download the agent and initialize the tunnel automatically.
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-3 pt-2">
                                        ...
                                        >
                                            <a href={platform === "python" ? "/agent/agent.py" : "/agent/agent.js"} download={platform === "python" ? "agent.py" : "agent.js"}>
                                                ...
                                                Download {platform === "python" ? "Python" : "Node.js"} Bridge
                                            </a>
                                        </Button>
