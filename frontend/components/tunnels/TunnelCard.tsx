...
interface TunnelCardProps {
    tunnel: Tunnel;
    onTokenRegenerated?: (rawToken: string, agentCommand: string) => void;
}
    ...
    const handleRegenerate = async () => {
        try {
            ...
            if (payload?.rawToken && onTokenRegenerated) {
                onTokenRegenerated(payload.rawToken, payload.agentCommand ?? "");
            }
        } catch (err: any) {
    ...
    return (
        <>
                ...
                <div className="flex items-center gap-1 shrink-0 bg-muted/50 p-1 rounded-xl border border-border relative z-10">
                    ...
                    >
                        <a
                            ...
                            title="Launch"
                            onClick={(e) => !tunnel.isActive && e.preventDefault()}
                        >
            ...
            <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
                <DialogContent className="sm:max-w-md rounded-3xl border-border bg-popover shadow-2xl p-8">
                    <DialogHeader>
                        <DialogTitle className="text-xl font-bold tracking-tight">Terminate Node?</DialogTitle>
                        <DialogDescription className="text-sm font-medium text-muted-foreground mt-2 leading-relaxed">
                            Confirm decommissioning of node <span className="font-mono text-foreground font-bold bg-muted px-1.5 py-0.5 rounded">/{tunnel.alias}</span>. This will permanently sever the neural uplink and revoke all access credentials.
                        </DialogDescription>
                    </DialogHeader>
